...
addUserMessage = function(message) {
    var place;
    if (lastPerson == 0) {
        // Need to get the message from before... and add a connector.
        place = document.getElementById('lastMessage');
        var connector = document.createElement('span');
        connector.classList = "blob-connector";
        place.appendChild(connector);
        place.setAttribute('id', null);

        place = document.getElementById('lastMessageBay');
    } else {
        ...
        place.classList = 'sender';
        message.classList = 'message';

        message.setAttribute('id', 'lastMessageBay');

        place = document.getElementById('lastMessageBay');
    }

    var blob = document.createElement('div');
    var blobContent = document.createElement('span');
    var msg = document.createElement('p');

    blob.classList = "blob";
    blobContent.classList = "blob-content"
    msg.innerText = message;

    blob.setAttribute('id', 'lastMessage');

    blob.appendChild(blobContent);
    blobContent.appendChild(msg);
...
window.onload = function () {
    ...
    $("#message_box").bind("enterKey", function (e) {
        addUserMessage(document.getElementById("message_box").value);
