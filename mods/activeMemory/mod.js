  ...
  if (mem.indexOf(_tokes[0].text) > -1) {
    ...
    switch(_tokes[0].text) {
        ...
        whoList.forEach(function(item) {
          if (message.indexOf(item) > -1){
            if (item in brain[socket.id]){
              ...
              return brain[socket.id][item]
            } else {
              return "Sorry, you havent told me yet, or i have simply forgotten."
            }
          }
        })
      ...
      case "what":
        whatList.forEach(function(item) {
          if (message.indexOf(item) > -1){
            console.log("Woooo! We found it: " + item)
            if (item in brain[socket.id]){
              console.log("So, it is in the brain... lets load it...")
              // We have found that the item was remembered...
              // going to return it now...
              console.log(brain)
              return brain[socket.id][item]
    ...
    whole_list.forEach(function(item) {
      if (message.indexOf(item) > -1) {
        ...
        return "I have remembered that now..."
      }
    })
